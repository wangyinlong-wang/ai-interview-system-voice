...
from sqlalchemy import Column, Integer, String, Text, DateTime, ForeignKey, JSON, Index, Boolean, Float
...
class Interview(Base):
    ...
    # 语音/3D 面试配置（新增）
    enable_voice = Column(Boolean, default=False)  # 是否启用语音模式
    enable_3d = Column(Boolean, default=False)     # 是否启用3D面试
    interviewer_model = Column(String(50), nullable=True, default="male")  # 面试官模型: male/female
    scene = Column(String(50), nullable=True, default="office")           # 场景: office/modern
...
class Evaluation(Base):
    ...
    question_reviews = Column(JSON, nullable=True)  # 逐题点评
    created_at = Column(DateTime, default=datetime.now)
...
class QuestionBank(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    title = Column(String(500), nullable=False)  # 题目内容
    answer = Column(Text, nullable=True)  # 参考答案
    analysis = Column(Text, nullable=True)  # 解析/思路
    category = Column(String(50), nullable=True)  # 岗位分类(frontend/backend/algorithm/product/...)
    sub_category = Column(String(50), nullable=True)  # 子分类(react/java/system_design/...)
    difficulty = Column(String(20), nullable=True)  # 难度(beginner/intermediate/advanced)
    question_type = Column(String(30), nullable=True)  # 题型(technical/behavioral/situational/algorithm)
    tags = Column(JSON, nullable=True)  # 标签数组
    source_name = Column(String(50), nullable=True)  # 来源名称
    source_url = Column(String(500), nullable=True)  # 来源URL
    view_count = Column(Integer, default=0)  # 查看次数
    use_count = Column(Integer, default=0)  # AI引用次数
    status = Column(String(20), default="pending")  # pending/approved/rejected
    crawled_at = Column(DateTime, default=datetime.now)  # 采集时间
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)
...
class CrawlTask(Base):
    ...
    source_type = Column(String(30), nullable=False)  # web/api/rss/github
    status = Column(String(20), default="running")  # running/success/no_data/failed
...
class CrawlSource(Base):
    ...
    source_type = Column(String(30), nullable=False)  # web/api/rss/github
    base_url = Column(String(500), nullable=False)
    spider_class = Column(String(100), nullable=False)  # Spider类名
    cron_expression = Column(String(50), default="0 2 * * *")  # 定时表达式，默认每天2点
    config = Column(JSON, nullable=True)  # 额外配置（如max_pages等）
    is_enabled = Column(Boolean, default=True)
